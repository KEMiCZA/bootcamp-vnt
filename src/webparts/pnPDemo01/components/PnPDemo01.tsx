...
export default class PnPDemo01 extends React.Component<IPnPDemo01Props, Demo01State> {
  private readonly _itemscount = 100;
  ...
  private async loadWebInformation() {
    ...
    const fields = await sp.web.fields.select('InternalName').filter('Hidden eq false and ReadOnlyField eq false and CanBeDeleted eq true').get();
  ...
  public render(): React.ReactElement<IPnPDemo01Props> {
    ...
    return (
      <div>
        ...
        <p><b>Fields:</b> ${fields.map(f => f.InternalName).join(', ')}</p>
